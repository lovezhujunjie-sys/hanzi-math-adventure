...
/* 2. 拼音必须有声调。
   🔴 例外是**真·轻声**——课本上本来就不标调，标了反而是错的：
      语气词（啊/嘛/呢/吧/吗/呀/哇/啦/哩）和结构助词「得」（跑得快）。
      2025 版生字表里出现了「得(de)」「哩(li)」两处轻声，所以白名单要跟着扩；
      但这不等于「不带调就放行」——不在这张表里的无调拼音照样报错。 */
const QING = new Set(['啊', '嘛', '呢', '吧', '吗', '呀', '哇', '啦', '哩', '得']);
...
/* 6. 抽几个课标里铁定有的字做哨兵——数据全丢/串册会立刻暴露。
   🔴 2025 版重排过册次：「村」挪到了二上第六单元，不再是二下的哨兵；
      换成同样铁定在二下的「蛙」（青蛙卖泥塘）、「雷」（雷雨）。
      哨兵字是「防数据整体丢失/串册」的探针，换册次就要跟着换，不然它会一直误报。 */
const SENTINEL = { '二上': ['两', '识', '庐', '瀑', '村'], '二下': ['诗', '童', '碧', '蛙', '雷'] };
